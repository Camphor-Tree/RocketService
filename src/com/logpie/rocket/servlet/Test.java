...
public class Test {
	...
	public static void main(String[] args) throws UnknownHostException,
		 ...
		 * System.out.println(db.getVersion()); DB rocketDB =
		 * db.getDB("Rocket"); Set<String> set = rocketDB.getCollectionNames();
		 * for(String a : set) { System.out.println(a); }
		 * //System.out.println(rocketDB.getStats()); //rocketDB.
		 */
		// testRocketWriteLog();
		testConnection();
		// testLogic();
	...
	private static void testLogic() throws JSONException {
		...
		// RocketServiceCentralLogic logic =
		// RocketServiceCentralLogic.getInstance();
		...
		for (MetricRecord record : list)
			RocketServiceCentralLogic.getInstance().insertRecordIntoMongoDB(
					record, new InsertCallback());
	...
	private static void testConnection() {
		Socket socket;
		try {
			socket = new Socket("localhost", 8443);
			OutputStream os;
			os = socket.getOutputStream();

			StringBuffer stringbuffer = new StringBuffer();
			stringbuffer.append("POST /RocketService/servlet HTTP/1.1\r\n");
			...
			String str = "{\"requestID\":\"1DEASEWO-2232GDA2\","
					...
					+ "\"timestamp\":9800284756389," + "\"time\":27}],"
					+ "\"mobile_device\":\"true\","
					+ "\"OS_type\":\"android\"," + "\"OS_version\":\"4.1\","
					+ "\"device_manufacture\":\"Samsung\","
					+ "\"device_version\":\"Galaxy S3\"}";
